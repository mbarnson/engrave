...
- [ ] **FNDN-04**: System supports multiple LLM providers (Anthropic API, OpenAI API, LMStudio local) via LiteLLM, configurable per pipeline stage
...
- [ ] **CORP-03**: RAG system retrieves relevant few-shot examples based on structured metadata (instrument family, ensemble type, style, musical context) to provide context for LilyPond generation
- [ ] **CORP-04**: Corpus is chunked at phrase level (4-8 bars) to expand 2K+ scores into 10K+ retrievable examples
