...
async def _run_midi_pipeline(input_path: Path, job_dir: Path) -> Path:
    """Run the MIDI-only pipeline path: generate -> render.
...
def _run_checks(
    ...
    Args:
        ...
        expected_instrument_count: Number of instruments in ensemble preset.
    ...
    results.append(check_all_parts_present(zip_path, expected_instrument_count))
    ...
    # Big band: 38-39 files (38 without MusicXML, 39 with)
    expected_min = 2 * expected_instrument_count + 3 + expected_instrument_count + 1
    # PDFs: 1 score + N parts = N+1
    # LY: 1 definitions + 1 score + N parts = N+2
    # MIDI: 1
    # Total without MusicXML: (N+1) + (N+2) + 1 = 2N+4
    # Total with MusicXML: 2N+5
    expected_min = 2 * expected_instrument_count + 4
    expected_max = 2 * expected_instrument_count + 5
    results.append(check_zip_file_count(zip_path, expected_min, expected_max))
...
async def run_smoke_test(test_dir: Path) -> SmokeResult:
    ...
    t0 = time.monotonic()
    expected_instrument_count = len(BIG_BAND.instruments)
    ...
    for input_path, pipeline_type in discovered:
        ...
        # Run checks
        checks = _run_checks(zip_path, error, expected_instrument_count)
